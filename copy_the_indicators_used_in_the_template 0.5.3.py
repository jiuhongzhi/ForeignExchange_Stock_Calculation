...
'''建立指标备份文件夹'''
a_path = 'f:\\Indicators_常用'                                  # 指标备份文件夹
if os.path.exists(a_path):                                      # 指标备份文件夹存在
    # print(a_path, 'folder already exists.')
    # message = '%s folder has been established.'
    message = 'The %s folder already exists.'
    print(message % a_path)
else:                                                           # 指标备份文件夹不存在
    # print(a_path, 'folder does not exist.')
    os.mkdir(a_path)                                            # 创建文件夹
    # print('The', a_path, 'folder has been created.')
    # message = '%s folder is not established. The %s folder is now established.'
    message = '%s folder does not exist. The %s folder has been created.'
    print(message % (a_path, a_path))
...
for line in result:
    ...
    if line.find('.tpl') >= 0:                                  # '.tpl'存在
        ...
        with open(path+'\\templates\\'+sourcefile, 'rb') as sf: # 按二进制方式打开源文件
            ...
            fileline = fileline.decode('gbk', 'replace')        # bytes 类型转换为 str 类型, 用�取代非法字符 (*.tpl文件内的中文是gb2312码,gb2312是gbk的子集)
            ...
            while fileline:                                                         # 非无内容行；无内容行(最后行)则退出
                ...
                if fileline.find('custom') >= 0:                                    # 'Custom'存在,则转换
                    ...
                    if fileline.find('expert') >= 0:                                # 'expert'存在,则转换
                        fileline = sf.readline()                                    # 再读1行
                        # print(chardet.detect(fileline))  # test     检查编码
                        # print(chardet.detect(fileline).get("encoding"))        # test  # 打印编码
                        # encoding_ = chardet.detect(fileline).get("encoding")   # test  # 获得编码
                        ...
                        if fileline.find('name=') >= 0:                             # 'name='存在,则转换
                            ...
                            if os.path.exists(files_to_copy + '.ex4'):                                          # 文件存在
                                ...
                                t = shutil.copy2(files_to_copy + '.ex4', a_path + '/')                          # 复制ex4文件
                            else:
                                b = '    ' + files_to_copy + '.ex4'
                                # a = files_to_copy  # test
                                # print(chardet.detect(a).get("encoding"))  # test
                                # print(chardet.detect(b))    # test
                                # print(type(b))  # test
                                # l = len(b.encode('gb2312'))                                                   # 按汉字为2字节计算长度
                                # l = len(b.encode(encoding_))                                                  # 按汉字为2字节计算长度
                                l = len(b.encode('gbk'))                                                        # 按汉字为2字节计算长度,gb2312是gbk的子集
                                l1 = 40 - l                                                                     # 空字节数
                                if l1 >= 5:                                                                     # 计算对齐位置
                            ...
                            if os.path.exists(files_to_copy + '.mq4'):                                          # 文件存在
                                ...
                                t = shutil.copy2(files_to_copy + '.mq4', a_path + '/')                          # 复制mq4文件
                            else:
                                d = '    ' + files_to_copy + '.mq4'
                                # print(type(d))  # test
                                # l = len(d.encode('gb2312'))                                                   # 按汉字为2字节计算长度
                                # l = len(d.encode(encoding_))          # test                                  # 按汉字为2字节计算长度
                                l = len(d.encode('gbk'))                                                        # 按汉字为2字节计算长度,gb2312是gbk的子集
                                l1 = 40 - l                                                                     # 空字节数
                                if l1 >= 5:                                                                     # 计算对齐位置
                ...
                # fileline = fileline.decode('gb2312', 'replace')       # bytes 类型转换为 str 类型, 用�取代非法字符 (*.tpl文件内的中文是gb2312码)
                fileline = fileline.decode('gbk', 'replace')            # bytes 类型转换为 str 类型, 用�取代非法字符 (*.tpl文件内的中文是gb码,gb2312是gbk的子集)
...
with open(a_path+'\\mq4ex4_files.txt', 'w', encoding='utf8') as tf:     # 按utf8码打开目标文件
    '''写入目标文件'''
