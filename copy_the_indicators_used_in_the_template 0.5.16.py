...
def backup_files(描述, sourcefile_path, 备份类型, objectfile_path, mqex_files):
    '''备份文件'''
    print('\n' + 描述 + ':')                                  # 显示
    mqex_files += '\n' + 描述 + ':\n'
    '''读源文件夹'''
...
def run_main():
    ...
    描述 = 'MT4 Expert Advisor files(mq4)'
    sourcefile_path = path+'\\MQL4\\Experts'
    备份类型 = '.mq4'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, e__path, mqex_files)  # 备份文件
    描述 = 'MT4 Expert Advisor files(ex4)'
    sourcefile_path = path+'\\MQL4\\Experts'
    备份类型 = '.ex4'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, e__path, mqex_files)  # 备份文件

    描述 = 'MT5 Expert Advisor files(mq5)'
    sourcefile_path = path5+'\\MQL5\\Experts'
    备份类型 = '.mq5'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, mt5_e_path, mqex_files)  # 备份文件
    描述 = 'MT5 Expert Advisor files(ex5)'
    sourcefile_path = path5+'\\MQL5\\Experts'
    备份类型 = '.ex5'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, mt5_e_path, mqex_files)  # 备份文件

    描述 = 'MT5 scripts custom files'
    sourcefile_path = path5+'\\MQL5\\Scripts'
    备份类型 = 'a_'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, mt5_s_path, mqex_files)  # 备份文件

    描述 = '部分MT4特殊指标文件备份'
    sourcefile_path = path+'\\MQL4\\Indicators'
    备份类型 = 'a_'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, i_path, mqex_files)  # 备份文件

    描述 = '部分MT4特殊脚本文件备份'
    sourcefile_path = path+'\\MQL4\\Scripts'
    备份类型 = 'a_'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, s__path, mqex_files)  # 备份文件
    ...
    备份类型 = '.mq5'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, mt5_i_path, mqex_files)  # 备份文件
    描述 = 'MQL5指标文件备份(ex5)'
    sourcefile_path = path5+'\\MQL5\\Indicators'
    备份类型 = '.ex5'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, mt5_i_path, mqex_files)  # 备份文件

    描述 = '模板内指标备份处理的Python程序文件备份'
    sourcefile_path = 'E:\\Documents\\workspace\\workspace_Python\\ForeignExchange_Stock_Calculation'
    备份类型 = '.py'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, py_path, mqex_files)  # 备份文件

    描述 = 'Visio流程图文件'
    sourcefile_path = 'E:\\Documents\\Visio_file'
    备份类型 = '.vsdx'
    mqex_files = backup_files(描述, sourcefile_path, 备份类型, visio_path, mqex_files)  # 备份文件
