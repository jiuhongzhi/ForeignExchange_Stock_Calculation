...
if os.path.exists(a_path):                                      # 指标备份文件夹存在
    ...
    print(message % a_path)                               # 显示
else:                                                           # 指标备份文件夹不存在
    os.mkdir(a_path)                                            # 创建文件夹
    message = 'The %s folder was not created. Now create the %s folder.'
    print(message % (a_path, a_path))                     # 显示
print('指标备份文件夹: ' + a_path + '\n')
mq4ex4_files += '(指标备份文件夹: ' + a_path + ')\n'             # 记录指标备份文件夹位置

'''建立模板备份文件夹'''
t_path = 'F:\\templates_常用'                                   # 模板备份文件夹
                    ...
                    if fileline.find('expert') >= 0:                                # 'expert'存在,则转换
                        ...
                        if fileline.find('name=') >= 0:                             # 'name='存在,则转换
                            ...
                            if os.path.exists(files_to_copy + '.ex4'):                                          # 文件存在
                                ...
                                print(a)                                                    # 显示
                                mq4ex4_files += a + '\n'                                                        # 加入有ex4文件记录
                                t = shutil.copy2(files_to_copy + '.ex4', a_path + '/')                          # 复制ex4文件，包含访问和修改时间
                            else:
                                b = '    ' + files_to_copy + '.ex4'
                                ...
                                if l1 >= 5:                                                                     # 计算对齐位置
                                    ...
                                    b = b.ljust(46 - l2) + 'no ex4 !!!!!!'                                      # 将'no ex4 !!!!!!'作对齐处理
                                else:
                                    b += '     no ex4 !!!!!!'
                                print(b)                                                    # 显示
                                mq4ex4_files += b + '\n'                                                        # 加入无ex4文件记录
                            if os.path.exists(files_to_copy + '.mq4'):                                          # 文件存在
                                ...
                                print(c)                                                    # 显示
                                mq4ex4_files += c + '\n'                                                        # 加入有mq4文件记录
                                t = shutil.copy2(files_to_copy + '.mq4', a_path + '/')                          # 复制mq4文件，包含访问和修改时间
                            else:
                                d = '    ' + files_to_copy + '.mq4'
...
'''遍历模板文件结束'''
print('\n\nTotal number of templates: ' + str(tpl_count))       # 显示
